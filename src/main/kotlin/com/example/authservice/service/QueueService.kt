...
class QueueService(
    ...
    fun enter(userId: String, eventId: String): QueueStatusResponse {
        ...
        val existingToken = redisTokenStore.getKey(entryTokenKey(userId))
        if (existingToken != null) {
            return QueueStatusResponse(
                status = "READY",
                position = 0,
                total = redisTokenStore.zcard(queueKey(eventId)),
                entryToken = existingToken,
            )
        }

        // 2. 이미 대기열에 있음 → 현재 순위 반환 (중복 진입 방지)
        val existingRank = redisTokenStore.zrank(queueKey(eventId), userId)
        ...
        redisTokenStore.zadd(queueKey(eventId), score, userId)

        val position = (redisTokenStore.zrank(queueKey(eventId), userId) ?: 0) + 1
        val total    = redisTokenStore.zcard(queueKey(eventId))
    ...
    fun status(userId: String, eventId: String): QueueStatusResponse {
        val entryToken = redisTokenStore.getKey(entryTokenKey(userId))
        if (entryToken != null) {
            return QueueStatusResponse(
                status = "READY",
                position = 0,
        ...
        val rank = redisTokenStore.zrank(queueKey(eventId), userId)
            ?: return QueueStatusResponse(
                status = "NOT_IN_QUEUE",
                position = null,
                total = redisTokenStore.zcard(queueKey(eventId)),
    ...
    fun release(userId: String, eventId: String) {
        val admittedEventId = redisTokenStore.getKey(admittedKey(userId)) ?: return
        redisTokenStore.delKey(admittedKey(userId))
        redisTokenStore.delKey(entryTokenKey(userId))  // 토큰이 이미 만료됐어도 no-op
        val newCount = redisTokenStore.decrKey(activeCountKey(admittedEventId))
    ...
    fun validateEntryToken(userId: String, entryToken: String): Boolean {
        val stored = redisTokenStore.getKey(entryTokenKey(userId))
    ...
    fun admitFromQueue(eventId: String) {
        val activeCount    = redisTokenStore.getKey(activeCountKey(eventId))?.toLongOrNull() ?: 0L
        ...
        val admitted = redisTokenStore.zpopmin(queueKey(eventId), slotsAvailable)
        if (admitted.isEmpty()) {
        ...
        for (userId in admitted) {
            val alreadyHasSlot = redisTokenStore.getKey(admittedKey(userId)) != null
            val token = UUID.randomUUID().toString()
            redisTokenStore.setKey(entryTokenKey(userId), token, ENTRY_TOKEN_TTL_SEC)
            redisTokenStore.setKey(admittedKey(userId), eventId, -1)
            if (alreadyHasSlot) {
                // 이전 슬롯이 남아있음 (entry만 만료된 경우) → INCR 스킵
                println("[QUEUE] re-admitted  userId=$userId  token=$token  (slot reused)")
            } else {
                val newCount = redisTokenStore.incrKey(activeCountKey(eventId))
