...
- 1,000/s 구간은 여유 (consumer 천장 미도달).

### 포화점 — consumer 처리 천장 (발행 rate 상승)

| 발행 rate | 실제 발행 | 소비 peak | 발행 중 백로그 | 수렴 |
|---|---|---|---|---|
| 2,000/s | 2,000/s | 3,594/s | 1,609 | 완전 (drain 1s) |
| **4,000/s** | 4,000/s | 4,060/s | **17,159** | 완전 (drain 6s) ⚠️ 경계 |
| **8,000/s** | 8,000/s | 3,788/s | **59,554** | **미수렴 (70,375/80,000)** ❌ |

- **발행자는 8,000/s까지 정상 발행**(rate_actual=8,000) → 발행자 병목 아님, **consumer가 병목**.
- 소비 peak가 3,594 → 4,060 → 3,788로 **~3,800/s에서 정체** = **consumer 처리 천장 ≈ 3,800 events/s**.
- **~2,000/s**: 실시간 수렴 · **4,000/s**: 경계(백로그 쌓였다 drain) · **8,000/s**: 백로그 폭증·미수렴(포화 초과).
- **병목 원인 추정**: ticket-query가 단일 폴링 스레드 + per-event `@Transactional` read_db upsert → read_db write가 ~3,800/s에서 막힘.
- **개선 방향**: 파티션 증설 + 다중 consumer(그룹 재배분), batch upsert.
- ⚠️ 한계: 발행자 단일·read_db polling 0.5s 해상도·동일 머신.

---

### 검증된 것 / 남은 것

- ✅ **CQRS 뒷단**: **~2,000 events/s까지 실시간 수렴**(손실 0, 전파지연 ~50–70ms), **consumer 처리 천장 ≈ 3,800/s** 실측(초과 시 백로그 폭증)
...
- ✅ CQRS 포화점: consumer 천장 ≈ 3,800 events/s 실측 (8,000/s 발행 시 백로그 폭증·미수렴)
- ⏳ CQRS consumer 천장 개선: 파티션 증설 + 다중 consumer / batch upsert 후 재측정
